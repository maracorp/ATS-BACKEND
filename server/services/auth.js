...
passport.use(
  new LocalStrategy(
    { usernameField: "email" }, // this tells Passport to expect `email` instead of `username`
    async (email, password, done) => {
      try {
        ...
        // Assuming you have a method like user.comparePassword
...
function login({ email, password, context }) {
  return new Promise((resolve, reject) => {
    passport.authenticate("local", (err, user) => {
      ...
      context.req.login(user, () => resolve(user));
    })({ body: { email, password } });
  });
}
